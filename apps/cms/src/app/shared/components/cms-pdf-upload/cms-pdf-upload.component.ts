...
@Component({
  selector: 'cms-pdf-upload',
  templateUrl: './cms-pdf-upload.component.html',
  styleUrls: ['./cms-pdf-upload.component.scss'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => CmsPdfUploadComponent),
      multi: true
    }
  ]
})
export class CmsPdfUploadComponent extends BBDBaseComponent implements OnInit, ControlValueAccessor {
  ...
  onUploadFile(target: any): void {
    ...
    // this.appAttachApiServ.uploadAppFileInfos(this.formData).subscribe({
    //   next: (res) => {
    //     if (!res) {
